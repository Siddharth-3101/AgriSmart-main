...
==========================================*/
  ...
  const farmerCount = usersList.filter((u) => u.role === "FARMER").length;
  const activeCropCount = crops.filter((c) => c.status === "ACTIVE").length;

  const stats = [
    {
      title: "Total Farmers",
      value: String(farmerCount || 2453),
      color: "#22c55e",
      icon: <FaUsers />,
      bg: "#dcfce7",
    },
    {
      title: "Total Farms",
      value: String(farms.length || 1765),
      color: "#2563eb",
      icon: <MdAgriculture />,
      bg: "#dbeafe",
    },
    {
      title: "Active Crops",
      value: String(activeCropCount || 29),
      color: "#f59e0b",
      icon: <FaLeaf />,
      bg: "#fef3c7",
    },
    {
      ...
      value: String(broadcastNotifications.length || 16),
      color: "#8b5cf6",
      icon: <FaClipboardList />,
      bg: "#ede9fe",
    },
  ...
  const recentFarmers = usersList
    .filter((u) => u.role === "FARMER")
    .slice(-3)
    .reverse();

  const mappedRecentUsers = recentFarmers.length > 0
    ? recentFarmers.map((f, index) => ({
        id: f.userId,
        name: f.name,
        village: f.district || "Hyderabad",
        crop: crops.find((c) => c.userId === f.userId)?.cropName || "Rice",
        image: `https://randomuser.me/api/portraits/men/${(f.userId || index) % 99}.jpg`
      }))
    : recentUsers;
